...
| nav_rth_climb_first | ON | If set to ON or SPIRAL craft will climb to nav_rth_altitude first before turning to head home. If set to OFF craft will turn and head home immediately climbing on the way. For a fixed wing ON will use a linear climb, SPIRAL will use a loiter turning climb with climb rate set by nav_auto_climb_rate, turn rate set by nav_fw_loiter_radius (SPIRAL is a fixed wing setting and behaves the same as ON for a multirotor) |
