    ...
    switch (batteryMetersConfig()->current.type) {
        ...
        case CURRENT_SENSOR_VIRTUAL:
            ...
            if (ARMING_FLAG(ARMED)) {
                // throttleStatus_e throttleStatus = calculateThrottleStatus(THROTTLE_STATUS_TYPE_RC);  // CR83
                ...
                if (allNav || autoNav) {    // account for motors running in Nav modes with throttle low + motor stop
                    ...
                    throttleOffset = (throttleStickIsLow() && feature(FEATURE_MOTOR_STOP)) ? 0 : (int32_t)rcCommand[THROTTLE] - 1000;    // CR83
                }
