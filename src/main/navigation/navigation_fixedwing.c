...
static bool autoSpeedIsActive = false;  // CR170
//
...
static void updatePositionHeadingController_FW(timeUs_t currentTimeUs, timeDelta_t deltaMicros)
{
    ...
    const pidControllerFlags_e pidFlags = PID_USING_HEADING | (errorIsDecreasing ? PID_SHRINK_INTEGRATOR : 0);  // CR171
...
static bool isAutoSpeedEnabled(void)
{
    ...
    bool autoSpeedIsDemanded = IS_RC_MODE_ACTIVE(BOXAUTOSPEED) || isAutoSpeedRequiredByNav() >= 0;  // CR170

    return ARMING_FLAG(ARMED) && autoSpeedIsDemanded && isProbablyStillFlying() && !thrStickEmergStop &&   // CR170
...
{
    if (!STATE(AIRPLANE) || !isAutoSpeedEnabled()) {  // CR170
    ...
    if (posControl.flags.horizontalPositionDataNew && posControl.flags.verticalPositionDataNew) {
        ...
        if (pitotGetValidForAirspeed()) {
            ...
            if (useAirSpeed || airspeedBoost) { // CR170
