...
// extern fpVector3_t          original_rth_home;         // the original rth home - save it, since it could be replaced by safehome or HOME_RESET  CR88
...
// extern uint32_t safehome_distance;               // distance to the nearest safehome    // CR88
...
typedef struct navConfig_s {
    ...
    struct {
        struct {
            // uint8_t use_thr_mid_for_althold;        // Don't remember throttle when althold was initiated, assume that throttle is at Thr Mid = zero climb rate  CR103
        ...
        uint8_t  cruise_yaw_rate;                   // Max yaw rate (dps) when CRUISE MODE is enabled       CR101
    } general;
    ...
    struct {
        ...
        uint8_t althold_throttle_type;          // throttle zero datum type for alt hold         CR103
    } mc;
...
extern uint16_t navDesiredHeading;   // CR104
