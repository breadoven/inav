...
typedef struct navConfig_s {
    ...
    struct {
        ...
        uint8_t  wp_tracking_accuracy;       // fixed wing tracking accuracy [m]   CR67
        uint8_t  wp_tracking_max_angle;      // fixed wing tracking accuracy max alignment angle [degs]   CR67
        uint8_t  wp_turn_smoothing;          // Smooths turns during WP missions    CR67
    } fw;
} navConfig_t;
