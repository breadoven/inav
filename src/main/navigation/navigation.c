...
PG_RESET_TEMPLATE(navConfig_t, navConfig,
    .general = {
        ...
        .auto_speed = SETTING_NAV_AUTO_SPEED_DEFAULT,                                 // speed in autonomous modes (3 m/s = 10.8 km/h)  CR45
        .max_auto_speed = SETTING_NAV_MAX_AUTO_SPEED_DEFAULT,                         // max allowed speed autonomous modes  CR45
    ...
    .fw = {
        ...
        .soaring_pitch_deadband = SETTING_NAV_FW_SOARING_PITCH_DEADBAND_DEFAULT,// pitch angle mode deadband when Saoring mode enabled  CR36
    }
);
...
float getActiveWaypointSpeed(void)
{
    if (posControl.flags.isAdjustingPosition) {
        ...
        uint16_t waypointSpeed = navConfig()->general.auto_speed;   // CR45
...
// CR6 xxxxxxxxxxxxxxxxxxxxx
{
    ...
    if (ARMING_FLAG(ARMED)) {
        ...
        if (navigationIsExecutingAnEmergencyLanding()) {
            if (autonomousNavNotPossible && (!emergLandingCancel || FLIGHT_MODE(FAILSAFE_MODE))) { // CR55
                return NAV_FSM_EVENT_SWITCH_TO_EMERGENCY_LANDING;
            }
        }
        // CR55
