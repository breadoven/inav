...
typedef enum {  // if changed update navFwLaunchStatus_e
    ...
    FW_LAUNCH_STATE_ABORTED,            // 11
    FW_LAUNCH_STATE_FLYING,             // 12
...
static inline bool isLaunchMaxAltitudeReached(void)
{
    ...
    return (navConfig()->fw.launch_max_altitude > 0) && ((getEstimatedActualPosition(Z) - getTakeoffAltitude()) >= navConfig()->fw.launch_max_altitude);  // CR138
}
...
static fixedWingLaunchEvent_t fwLaunchState_FW_LAUNCH_STATE_IDLE_WIGGLE_WAIT(timeUs_t currentTimeUs) {
    if (throttleStickIsLow() && !navConfig()->fw.launch_allow_throttle_low) {   // CR6
