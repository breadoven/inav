...
static void virtualPitotCalculate(pitotDev_t *pitot, float *pressure, float *temperature)
{
    ...
    if (pitotIsCalibrationComplete()) {
        ...
        if (isEstimatedWindSpeedValid() && STATE(GPS_FIX)) {
            ...
            // horizontalWindSpeed *= cos_approx(DECIDEGREES_TO_RADIANS(attitude.values.pitch));   // correct for pitch
            // airSpeed = posControl.actualState.velXY - horizontalWindSpeed; //float cm/s or gpsSol.groundSpeed int16_t cm/s  CR165
            // airSpeed = calc_length_pythagorean_2D(airSpeed, getEstimatedActualVelocity(Z) + sin_approx(DECIDEGREES_TO_RADIANS(attitude.values.pitch)) * getEstimatedWindSpeed(Z));
        }
