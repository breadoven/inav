...
static const blackboxDeltaFieldDefinition_t blackboxMainFields[] = {
    ...
    {"fwAutoSpeedP",   -1, SIGNED,   .Ipredict = PREDICT(0),       .Iencode = ENCODING(SIGNED_VB),   .Ppredict = PREDICT(PREVIOUS),      .Pencode = ENCODING(SIGNED_VB), CONDITION(FIXED_WING_NAV)},  // CR164
...
static void writeIntraframe(void)
{
    ...
    if (testBlackboxCondition(CONDITION(FIXED_WING_NAV))) {
        ...
        blackboxWriteSignedVB(blackboxCurrent->fwAutoSpeedP);   // CR164
...
static void writeInterframe(void)
{
    ...
    if (testBlackboxCondition(CONDITION(FIXED_WING_NAV))) {
        ...
        blackboxWriteSignedVB(blackboxCurrent->fwAutoSpeedP - blackboxLast->fwAutoSpeedP);  // CR164
...
static void loadMainState(timeUs_t currentTimeUs)
{
    ...
    if (STATE(FIXED_WING_LEGACY)) {
        ...
        blackboxCurrent->fwAutoSpeedP = lrintf(nav_pids->fw_autoSpeed.proportional);    // CR164
